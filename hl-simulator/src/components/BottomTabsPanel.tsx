  ...
  return (
    <div className="h-full flex flex-col bg-s1 border-t border-brd">
      ...
      <div className="flex-1 overflow-auto">
        {activeTab === "balances" && (
          <BalancesContent balance={balance} />
        )}
        {activeTab === "positions" && (
          <PositionsContent
            positions={positions}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-s2 border border-brd rounded-lg p-5 w-[340px]">
            ...
            <div className="text-[12px] text-t2 mb-3">
              {closeModal.side} {closeModal.coin}-USD | Size: {closeModal.size}
            </div>
...
function HistoryContent({ history }: { history: TradeHistory[] }) {
  if (history.length === 0) {
    return (
      <div className="text-center py-8 text-t3 text-[11px]">No trade history yet</div>
    );
  }
